...
export class HoverDocsProvider implements HoverProvider {
  ...
  async provideHover(
    ...
    await new Promise(resolve => setTimeout(resolve, HoverDocsProvider.DEBOUNCE_MS));
    ...
    const markdown = HoverDocsProvider.getMarkdownSummary(summaryData);
  ...
  private static getMarkdownSummary(data: SummaryResponse | null): MarkdownString {
    ...
    if (!data) {
      markdown.appendMarkdown("**HoverDocs:** [Sign in](command:hoverdocs.signin) to view summary.");
      return markdown;
    }
    ...
    markdown.appendMarkdown("### HoverDocs AI\n\n");
